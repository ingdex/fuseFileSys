		     ...
		     off_t offset, struct fuse_file_info *fi)
{
	...
	if( size ==131072 ){
		while( size - inoff >= 4088 ){//如果还有一整块
			...
			res = pwrite(fd, FRM, 4 ,0);
			res = pwrite(fd, FRM, 4 ,0);//写入FRM
		}
		res = pwrite(fd, buf, size-offset-inoff, offset+inoff);
	}
	
	//res = pwrite(fd, buf, size-inoff, offset+inoff);
	else{
		while( size - inoff >= 4088 ){//如果还有一整块
			...
			res = pwrite(fd, FRM, 4 ,0);
			res = pwrite(fd, FRM, 4 ,0);//写入FRM BLK
		}
		res = pwrite(fd, buf, size-offset-inoff, offset+inoff);
		for(int i=5;i<8;i++){
			FRM[i] = 0xff; 
